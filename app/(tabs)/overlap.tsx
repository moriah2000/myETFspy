...
import { calculateSimpleReturn, hasHistoricalPrices } from '../hooks/useSnapshotEngine';
...
const PERFORMANCE_ENABLED = true;
...
function AddAssetModal({ visible, onClose, onAdded, existingPositions, addTransaction, transactions }: AddAssetModalProps) {
  ...
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    useEffect(() => {
    if (visible) { setStep('search'); setQuery(''); setResults([]); setSelected(null); setQty(''); setAvgCost(''); setPurchaseDate(''); setFees(''); setIsExisting(false); setTransactionType('BUY'); }
  }, [visible]);
...
async function handleSave() {
  ...
  if (transactionType === 'SELL') {
    const tickerTxns = transactions.filter(t => t.ticker === selected.ticker.toUpperCase());
    const pos = calculatePositionFIFO(selected.ticker.toUpperCase(), tickerTxns, 0);
    const held = pos?.totalShares ?? 0;
    if (qtyNum > held) {
      alert(`Cannot sell ${qtyNum} shares — you only hold ${held.toFixed(4)} shares of ${selected.ticker.toUpperCase()}.`);
      return;
    }
  }
  setSaving(true);
  try {
    ...
    await addTransaction({
      ...
      fees: feesNum,
      date: (() => {
        const d = purchaseDate.trim();
        if (!d) return undefined;
        // Reject anything that isn't YYYY-MM-DD
        ...
        <View style={modal.sheet}>
          ...
          {step === 'search' ? (
            ...
            <>
              ...
              <Text style={modal.inputLabel}>{transactionType === 'SELL' ? 'Sale Price per Share' : 'Price per Share'}</Text>
              ...
              <Text style={modal.inputLabel}>Fees / Commission (optional)</Text>
              <TextInput style={modal.input} placeholder="e.g. 4.99" placeholderTextColor="#4A6080" value={fees} onChangeText={setFees} keyboardType="decimal-pad" />
              <TouchableOpacity
                style={[modal.saveBtn, { backgroundColor: transactionType === 'SELL' ? '#FF5A5F' : '#338DFF' }, (!qty || saving) && { opacity: 0.5 }]}
                onPress={handleSave}
                disabled={!qty || !avgCost || saving}
              >
...
export default function PortfolioScreen() {
  ...
  // TEMPORARY: hard-clear stale snapshot cache on first mount
  // Remove this block once confirmed working
  const hardClearedRef = React.useRef(false);
  if (!hardClearedRef.current) {
    hardClearedRef.current = true;
    AsyncStorage.multiRemove(['portfolio_daily_snapshots', 'portfolio_daily_snapshots_meta']);
  }

  const { snapshots, rebuilding: snapshotsRebuilding, priceHistoryMap, forceRebuild } = usePortfolioSnapshots(transactions, transactionsReady, PERFORMANCE_ENABLED);

  // One-time snapshot cache clear — removes stale snapshots built before
  // historical prices were available. Remove this effect after first clean run.
  const snapshotClearedRef = React.useRef(false);
  useEffect(() => {
    if (!snapshotClearedRef.current && transactionsReady) {
      snapshotClearedRef.current = true;
      forceRebuild();
    }
  }, [transactionsReady]);
  const priceHistoryAvailable = Object.values(priceHistoryMap).some(h => (h as any[]).length > 0);
  ...
  // TWR requires historical prices. Without them cumulativeReturn is meaningless
  // (all past portfolio values are $0, producing absurd percentages).
  // Fall back to simple return which is always accurate from the transaction ledger.
  const twrAvailable = hasHistoricalPrices({});  // will be true once price history API is wired
  const displayReturn = twrAvailable && latestSnapshot
    ? latestSnapshot.cumulativeReturn
    : calculateSimpleReturn(totalValue, latestSnapshot?.contributions ?? 0);
  ...
  usePortfolioChartPoints({
    ...
    liveContributions: latestSnapshot?.contributions,
  ...
  return (
            ...
            {latestSnapshot && (
              <View style={s.metricsGrid}>
                ...
                <View style={s.metricCell}>
                  ...
                  <Text style={[s.metricValue, { color: displayReturn >= 0 ? '#00C896' : '#FF5A5F' }]}>
                    {displayReturn >= 0 ? '+' : ''}{(displayReturn * 100).toFixed(2)}%
                  </Text>
                </View>
                <View style={s.metricCell}>
    ...
    />
      <AddAssetModal visible={showAddAsset} onClose={() => setShowAddAsset(false)} onAdded={handlePortfolioChange} existingPositions={positions} addTransaction={addTransaction} transactions={transactions} />
