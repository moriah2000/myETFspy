...
function portfolioValueOnDate(
  ...
  for (const txn of transactions) {
    ...
    if (txn.transactionType === 'BUY' || txn.transactionType === 'TRANSFER_IN' || txn.transactionType === 'DRIP') {
...
function netContributionsOnDate(transactions: Transaction[], date: string): number {
  // Build per-ticker lot state so we can clamp SELL quantities to what FIFO
  // actually consumed. A raw txn.quantity on a SELL can exceed shares held
  // (oversell), which would make contributions go negative if unclamped.
  const lotsByTicker: Record<string, { quantity: number }[]> = {};
  ...
  for (const txn of relevant) {
    if (txn.transactionType === 'BUY' || txn.transactionType === 'TRANSFER_IN' || txn.transactionType === 'DRIP') {
      if (!lotsByTicker[txn.ticker]) lotsByTicker[txn.ticker] = [];
      const feePerShare = txn.quantity > 0 ? (txn.fees ?? 0) / txn.quantity : 0;
      lotsByTicker[txn.ticker].push({ quantity: txn.quantity });
      net += txn.quantity * txn.pricePerShare + (txn.fees ?? 0);
    } else if (txn.transactionType === 'SELL' || txn.transactionType === 'TRANSFER_OUT') {
      const lots = lotsByTicker[txn.ticker] ?? [];
      // Clamp sell qty to what is actually held — mirrors FIFO engine behaviour
      let toSell = txn.quantity;
      let effectiveQty = 0;
      while (toSell > 0 && lots.length > 0) {
        const lot = lots[0];
        if (lot.quantity <= toSell) {
          effectiveQty += lot.quantity;
          toSell -= lot.quantity;
          lots.shift();
        } else {
          effectiveQty += toSell;
      ...
      lotsByTicker[txn.ticker] = lots;
      // Only subtract proceeds for shares actually sold, not the phantom excess
      net -= effectiveQty * txn.pricePerShare - (txn.fees ?? 0);
    }
  }
  return net;
}
...
function cumulativeRealizedGainOnDate(transactions: Transaction[], date: string): number {
  ...
  for (const ticker of tickers) {
    ...
    for (const txn of tickerTxns) {
      if (txn.transactionType === 'BUY' || txn.transactionType === 'TRANSFER_IN' || txn.transactionType === 'DRIP') {
        const feePerShare = txn.quantity > 0 ? (txn.fees ?? 0) / txn.quantity : 0;
        lots.push({ quantity: txn.quantity, pricePerShare: txn.pricePerShare + feePerShare });
      } else if (txn.transactionType === 'SELL' || txn.transactionType === 'TRANSFER_OUT') {
        let toSell = txn.quantity;
        while (toSell > 0 && lots.length > 0) {
          const lot = lots[0];
...
// ─────────────────────────────────────────────
// Check whether priceHistoryMap has any usable historical data.
// If empty or all histories are empty, TWR cannot be calculated meaningfully.
// ─────────────────────────────────────────────

export function hasHistoricalPrices(priceHistoryMap: PriceHistoryMap): boolean {
  return Object.values(priceHistoryMap).some(h => h.length > 0);
}

// ─────────────────────────────────────────────
// Simple return — used as fallback when historical prices are unavailable.
// simpleReturn = (currentValue - contributions) / contributions
// Always accurate because it only uses live prices + transaction ledger.
// ─────────────────────────────────────────────
...
export function calculateSimpleReturn(
  ...
  return (currentValue - contributions) / contributions;
}
